...
class KittiViewer(QMainWindow):
    ...
    def draw_compare_in_image(self):
        if self.kitti_info is None:
            self.error("you must load infos and choose a existing image idx first.")
            return
    ...
    def draw_gt_in_image(self):
        ...
        boxes_3d_p2 = boxes_3d_p2.reshape([-1, 8, 2])
        if self.current_image is not None:
            bbox_plot.draw_3d_bbox_in_ax(self.w_plt.ax, boxes_3d_p2, colors="r")
    ...
    def draw_detection(self, detection_anno, label_color=GLColor.Blue):
        ...
        detection_anno = kitti.remove_low_score(
            detection_anno, self.w_config.get("DTScoreThreshold")
        )
        ...
        if self.w_config.get("DTScoreAsAlpha") and scores is not None:
            dt_box_color = np.concatenate(
                [dt_box_color[:, :3], scores[..., np.newaxis]], axis=1
            )
        self.w_pc_viewer.boxes3d(
            "dt_boxes",
            dt_boxes_corners,
            dt_box_color,
            self.w_config.get("DTBoxLineWidth"),
            1.0,
        )
    ...
    def plot_gt_boxes_in_pointcloud(self):
        ...
        if "annos" in self.kitti_info:
            ...
            labels_ = [
                f"{i}:{l}, {d}" for i, l, d in zip(label_idx, self.gt_names, diff_names)
            ]
            boxes_corners = box_np_ops.center_to_corner_box3d(
                self.gt_boxes[:, :3],
                self.gt_boxes[:, 3:6],
            ...
            self.w_pc_viewer.boxes3d("gt_boxes", boxes_corners, gt_box_color, 3.0, 1.0)
    ...
    def plot_compare_boxes_in_pointcloud(self):
        if self.kitti_info is None:
            self.error("you must load infos and choose a existing image idx first.")
            return
    ...
    def plot_pointcloud(self):
        ...
        if "annos" in self.kitti_info:
            ...
            gt_point_mask = box_np_ops.points_in_rbbox(self.points, self.gt_boxes).any(
                1
            )
    ...
    def plot_pointcloud_segmentation(self):
        if self.kitti_info is None:
            self.error("you must load infos and choose a existing image idx first.")
            return

        color_none = np.array((0.5, 0.5, 0.5))
        color_obj = np.array((0, 0.5, 0))
        color_ground_truth = np.array((0.5, 0, 0))

        # self.points[0:1000, -2:-1] = 1
        # self.points[1000:2000, -2:-1] = 0.7
        # self.points[2000:3000, -2:-1] = 0.5
        # self.points[3000:4000, -2:-1] = 0.25
    ...
    def load_info(self, image_idx):
        ...
        if self.kitti_detection_files is None:
            self.kitti_detection_files = get_kitti_detection_files()

        self.kitti_info = create_detection_kitti_info(
            arrays_dict_element(self.kitti_detection_files, image_idx), image_idx
        )
        ...
        if "img_path" in self.kitti_info:
            ...
            self.current_image = None
        v_path = str(self.kitti_info["velodyne_path"])

        num_features = 4
        if "pointcloud_num_features" in self.kitti_info:
            num_features = self.kitti_info["pointcloud_num_features"]

        points = np.fromfile(v_path, dtype=np.float32, count=-1).reshape(
            [-1, num_features]
        )
        self.points = points
        rect = self.kitti_info["calib/R0_rect"]
        ...
        if "annos" in self.kitti_info:
            ...
            self.gt_boxes = box_np_ops.box_camera_to_lidar(gt_boxes_camera, rect, Trv2c)
        ...
        if compare_results_detection is not None:

            for method in compare_results_detection_methods:
                method_name = method['name']
                method_color = method['color']
                
                annos = get_compare_detection_annotation(
                    method_name, image_idx
                )
                labels = annos["name"]
                num_obj = len([n for n in annos["name"] if n != "DontCare"])
                # print(annos["group_ids"].shape)
    ...
    def load_info_tracking(self, group_idx, image_idx):
        ...
        if self.kitti_tracking_files is None:
            self.kitti_tracking_files = get_kitti_tracking_files()

        self.kitti_info = create_tracking_kitti_info(
            arrays_dict_element(self.kitti_tracking_files, group_idx), image_idx
        )

        self.json_setting.set("image_idx", str(image_idx))
        self.json_setting.set("group_idx", str(group_idx))
    ...
    def load_info_segmentation(self, image_idx):
        ...
        if self.kitti_segmentation_files is None:
            self.kitti_segmentation_files = get_kitti_segmentation_files()

        self.kitti_info = create_segmentation_kitti_info(
            self.kitti_segmentation_files[image_idx]
        )

        self.json_setting.set("image_idx", str(image_idx))

        self.points = self.kitti_info['points']
    ...
    def plot_image(self):
        ...
        if self.current_image is not None:
            ...
            if "annos" in self.kitti_info:
                annos = self.kitti_info["annos"]
                annos = kitti.filter_kitti_anno(annos, self.w_config.get("UsedClass"))
                labels = annos["name"]
                num_obj = len([n for n in annos["name"] if n != "DontCare"])
    ...
    def plot_all_tracking(self, group_idx, image_idx):
        self.w_plt.reset_plot()
    ...
    def plot_all_segmentation(self, image_idx):
        self.w_plt.reset_plot()
        self.load_info_segmentation(image_idx)
        self.plot_pointcloud_segmentation()
    ...
    def on_plotButtonPressed_tracking(self):
        image_idx = int(self.w_imgidx_tracking.text())
        group_idx = int(self.w_groupidx_tracking.text())
        if self.plot_all_tracking(group_idx, image_idx):
            self.current_idx = image_idx
            self.current_group_idx = group_idx
    
    def on_plotButtonPressed_segmentation(self):
        image_idx = int(self.w_imgidx_segmentation.text())
        if self.plot_all_segmentation(image_idx):
            self.current_idx = image_idx
    ...
    def on_EvalVxNetPressed(self):
        ...
        with self.inference_ctx.ctx():
            for info in list_bar(self.kitti_infos):
                ...
                inputs = self.inference_ctx.get_inference_input_dict(info, points)
                det_annos += self.inference_ctx.inference(inputs)
    ...
    def get_false_pos_neg(gt_boxes, dt_boxes, labels, fp_thresh=0.1):
        ...
        if len(gt_boxes) != 0 and len(dt_boxes) != 0:
            ...
            ret[
                np.logical_and(
                    iou_max_dt_for_gt < tp_thresh, iou_max_dt_for_gt > fp_thresh
                )
            ] = 1  # FP
